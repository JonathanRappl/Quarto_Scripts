...
![Schichtenmodell-Analogie: Kommunikation zwischen zwei Firmen](figures/schichtenmodell_analogie.png){fig-align="center" width=75%}

<!-- TikZ source (pre-rendered to figures/schichtenmodell_analogie.png):
To regenerate, compile networking/figures/schichtenmodell_analogie.tex with pdflatex,
then convert: pdftoppm -png -r 300 schichtenmodell_analogie.pdf schichtenmodell_analogie
-->

![](figures/schichtenmodell_analogie.tex){fig-align="center" width=75%}
...
|---|---|---|---|---|---|
| 7 | Anwendung (*Application*) | Anwendung | Hilft bei der Identifizierung des Clients und synchronisiert die Kommunikation. | Nachricht | HTTP, HTTPS, DNS, SMTP, FTP |
| 6 | Darstellung (*Presentation*) | Anwendung | Daten werden extrahiert und für die Übertragung in das erforderliche Format gebracht. | Nachricht | TLS, SSL, ASCII, UTF-8 |
| 5 | Sitzung (*Session*) | Anwendung | Stellt Verbindung her, hält diese aufrecht, gewährleistet Authentifizierung und Sicherheit. | Nachricht | TLS Handshake |
| 4 | Transport | Transport | Nimmt Dienst von der Vermittlungsschicht und stellt ihn der Anwendungsschicht zur Verfügung. | Segment | TCP, UDP, Ports |
| 3 | Vermittlung (*Network*) | Internet | Übertragung von Daten von einem Host zu einem anderen in unterschiedlichen Netzwerken. | Paket | IPv4, IPv6, ICMP, Router, Gateway |
| 2 | Sicherung (*Data Link*) | Netzzugriff | Übermittlung von Nachrichten von Knoten zu Knoten. | Frame | ARP, MAC, Switch, Ethernet, WiFi |
| 1 | Bitübertragung (*Physical*) | Netzzugriff | Herstellung physischer Verbindungen zwischen Geräten. | Bits | Hub, Repeater, Modem, Kabel |

: Internet-Schichten-Modelle OSI und TCP/IP sowie deren wichtigste Aufgaben, Informationsformen, Geräte und Protokolle {#tbl-osi-layers}
...
```
┌─────────────────────────────────────────────────────────────────────────┐
│ Netzzugriff (MAC zu MAC)                                              │
│  ┌───────────────────────────────────────────────────────────────┐     │
│  │ Vermittlung (IP zu IP)                                       │     │
│  │  ┌─────────────────────────────────────────────────────┐     │     │
│  │  │ Transport (Port zu Port)                            │     │     │
│  │  │  ┌───────────────────────────────────────────┐      │     │     │
│  │  │  │ Anwendung (Client zu Server)              │      │     │     │
│  │  │  │              Nutzdaten                    │      │     │     │
│  │  │  └───────────────────────────────────────────┘      │     │     │
│  │  └─────────────────────────────────────────────────────┘     │     │
│  └───────────────────────────────────────────────────────────────┘     │
└─────────────────────────────────────────────────────────────────────────┘
 ◄──────── Header (Kopfteil) ────────►◄──── Payload (Nutzteil) ────►
```
...
b) Für **jede** Schicht:
   ...
   - Notieren Sie **wer mit wem** kommuniziert (z.B. *Kunde ↔ Digitec*, *Digitec ↔ Logistikpartner*, *Lager ↔ Zusteller*, ...).
...
- `192.168.0.33` sendet daher eine Nachricht an alle Computer in `192.168.0.x` (*Broadcast*): «Wer hat die IP `192.168.0.34`?»
- Computer `192.168.0.34` antwortet (*Unicast*): «Ich! Meine MAC ist `60:4E:46:F5:08:09`.»
- Nun kann der Computer seine Nachricht direkt an den nächsten Empfänger (*Gateway*) übergeben.

Die Resultate aller ARP-Anfragen werden in einer sogenannten **ARP-Tabelle** gespeichert, damit nicht jedes Mal eine neue Anfrage gesendet werden muss. Diese Tabelle wird periodisch aktualisiert. Um die ARP-Tabelle auf einem Computer anzuzeigen, kann der Befehl `arp -a` im Terminal (MacOS, UNIX) bzw. `cmd` (Windows) verwendet werden.
...
Der `ping`-Befehl kann im Terminal (MacOS, UNIX) bzw. `cmd` (Windows) wie folgt ausgeführt werden:
...
1. Verbinden Sie in Filius drei Rechner physisch mithilfe eines Switches. Stellen Sie IPs und Netzmasken so ein, dass die Geräte auch logisch in ein Subnetz passen, z.B. `1.2.3.x`. Überprüfen Sie mithilfe von Pings, dass die Einstellungen funktionieren.
...
$2^{32} \approx 4 \text{ Milliarden Adressen}$
...
- `2001:db8:::1` — ❌ Keine gültige IP-Adresse (mehrfache Verwendung von `::`)
- `192.168.1.1.1` — ❌ Keine gültige IP-Adresse (zu viele Oktette)
:::
...
| 10.0.0.0 | 255.255.255.252 | 10.0.0.1 | Ja ($252_{10}=11111100_2$) |
| 1.2.3.0 | 255.255.255.252 | 1.2.3.5 | Nein ($252_{10}=11111100_2$) |
:::
