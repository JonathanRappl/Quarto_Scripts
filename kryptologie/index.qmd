...
<!-- ::: {.callout-note title="Aufgabe 10: Entschlüsseln" icon=true}
...
<!-- ## Ausblick: Kasiski-Test

Die Vigenère-Verschlüsselung blieb fast drei Jahrhunderte lang ungebrochen — bis Friedrich Kasiski 1863 eine systematische Methode zur Bestimmung der Schlüssellänge veröffentlichte.

**Die Idee:** Wenn dasselbe Klartextwort zufällig an zwei verschiedenen Positionen steht, bei denen der Abstand ein Vielfaches der Schlüssellänge $m$ ist, entsteht **im Chiffretext dieselbe Buchstabenfolge**. Findet man solche Wiederholungen, verraten deren Abstände die Schlüssellänge.

**Vorgehen (Kasiski-Test):**

1. **Wiederholungen suchen:** Alle im Chiffretext mehrfach vorkommenden Zeichenfolgen der Länge ≥ 3 finden und deren Abstände notieren.
2. **GGT berechnen:** Den grössten gemeinsamen Teiler (GGT) aller gefundenen Abstände berechnen — dieser ist wahrscheinlich die Schlüssellänge $m$.
3. **Text aufteilen:** Den Chiffretext in $m$ Teilfolgen aufteilen: Teilfolge 1 enthält alle Buchstaben an Position 1, $m+1$, $2m+1$, …; Teilfolge 2 alle an Position 2, $m+2$, $2m+2$, … usw.
4. **Häufigkeitsanalyse:** Jede Teilfolge ist ein Caesar-Cipher mit festem Schlüssel → klassische Häufigkeitsanalyse anwenden.

**Beispiel:** Im Chiffretext findet man die Folge `XQR` an Position 4 und an Position 22 (Abstand 18) sowie an Position 34 (Abstand 30 von der ersten). GGT(18, 30) = 6 → wahrscheinliche Schlüssellänge: **6**.

::: {.callout-warning title="Aufgabe 11 (Fortgeschrittene): Kasiski-Test anwenden" icon=true}
Der folgende Text wurde mit Vigenère und einem **unbekannten** Schlüsselwort verschlüsselt.

```
[LÜCKE: Hier längeren Vigenère-Chiffretext einfügen]
```

**Schritt 1 — Schlüssellänge bestimmen:**

Suchen Sie von Hand (oder mit der Suchfunktion des Browsers) nach wiederholten Drei-Buchstaben-Folgen im Chiffretext. Notieren Sie die Abstände und berechnen Sie den GGT.

**Schritt 2 — Teilfolgen analysieren:**

Extrahieren Sie die erste Teilfolge (Buchstaben an Position 1, $m+1$, $2m+1$, …) und analysieren Sie die Häufigkeiten. Welchen Caesar-Schlüssel ergibt die Häufigkeitsanalyse?

**Schritt 3 — Schlüsselwort rekonstruieren:**

Wiederholen Sie Schritt 2 für alle $m$ Teilfolgen und setzen Sie die Schlüsselbuchstaben zusammen. Überprüfen Sie das Schlüsselwort mit dem Widget oben.

::: {.callout-tip title="Hilfsmittel" collapse=true icon=true}
[CyberChef – Vigenère Analyse](https://gchq.github.io/CyberChef/#recipe=Vigen%C3%A8re_Decode('')){target="_blank"} (Schlüssel manuell eingeben und prüfen)

[dcode.fr – Kasiski-Test](https://www.dcode.fr/kasiski-test){target="_blank"} (automatische Schlüssellängenbestimmung) 
:::
